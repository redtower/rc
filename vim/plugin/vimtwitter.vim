...
function! s:Twitter()
    ...
    elseif strlen(s:tweet) <= 140
        ...
        call system("curl -u redtowers:mori123 -d status=\"" . s:tweet . "\" http://twitter.com/statuses/update.xml?source=vim")
...
let s:currentline = getline('.')
    ...
    elseif strlen(s:currentline) <= 140
        ...
        call system("curl -u redtowers:mori123 -d status=\"" . s:currentline . "\" http://twitter.com/statuses/update.xml?source=vim")
...
function! s:CmdLine_Twitter()
    ...
    elseif strlen(s:cmdline_twitter) <= 140
        ...
        call system("curl -u redtowers:mori123 -d status=\"" . s:cmdline_twitter . "\" http://twitter.com/statuses/update.xml?source=vim")
